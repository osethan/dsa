...
let biggerThanLuke = (arr) => {
  ...
  // Get Luke's mass to compare
  const luke = arr.find(el => {
  ...
  // Compare masses to Luke's mass
  const biggerMasses = arr.filter(el => {
    if (Number(el.mass) > Number(lukeMass)) {
      return el.name;
    }
  }).map(el => {
    return el.name;
  });
  const biggerMassesStr = biggerMasses.join(' - ');

  return biggerMassesStr;
};
